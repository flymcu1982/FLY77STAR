...
5. **`IMAGE_GENERATION_POLICY.md`**(Production Policy Version 1.1) — 画像生成エンジン標準・AI社員の役割定義・制作フロー・Panel04以降の生成実行ルール
...
## Production Policy Version 1.1の理念(最優先事項)

> FLY77STARの価値は、AIが作品を作ることではない。
> 人間のクリエイティブを、AIによって最大限に表現することである。

AI社員(Codex含む)は原則**「生成する担当」ではなく「最高品質の制作設計を行う担当」**である。担当範囲はStory Bible / Character Master / Location Master / Panel Storyboard / Director Notes / Image Prompt / QC / Production Review / Production Report / 登録。**実際の本番画像生成は原則としてAI社員が行わない。**

**例外(Version 1.1、2026-07-11〜)**: WAKE UP Panel04以降に限り、社長の明示的な方針変更指示により、AI社員が下記「厳格運用サイクル」のもとで本番画像生成を実行する。この例外は都度の明示的な社長指示によってのみ他のカット・他のプロジェクトへ拡張する。詳細: `IMAGE_GENERATION_POLICY.md`。

## Codex固有の厳守事項

- **画像生成を実行する場合(WAKE UP Panel04以降等、社長が明示的に許可した範囲のみ)は、厳格運用サイクルを厳守する**:
  1. 1テーク(1カット)生成したら必ず停止し、Director Reviewを待つ
  2. 次のカットには、Director(YU)からの明示的な「GO」「次へ」の指示があるまで進まない
  3. 生成後は毎回、Character/Composition/Lighting/Story/Emotionの5項目で自己チェック結果を報告する
  4. 承認された場合のみSTATUS.md/Daily Studio Report/Vault記録/コミットを実施する
- **上記の許可範囲外では、社長承認前に画像生成を実行しない。** GPT Image/Higgsfieldでの生成は、制作フロー(`IMAGE_GENERATION_POLICY.md`参照)の②社長レビュー・③採用決定を経てからの工程であり、明示的な許可なくAI社員が独断で実行することはない
