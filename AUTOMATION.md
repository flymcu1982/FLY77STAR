...
CINE→SOUL→CUT→MASTERの4段階リレーによるCUT単位制作の標準フローは `AI_WORKFLOW_V1.md` を参照。すべてのMV・ドラマ・映画で共通運用する(2026-07-10、WAKE UP CUT01の試験運用を経て正式運用開始)。
...
- `scripts/vault_manager.py`: 制作中に決まった内容を `Obsidian_Vault/` の7区画(Daily Log/Discovery/Decision Log/Character Bible/Costume Bible/MV Bible/Prompt Library)へ整理・保存する(他のスクリプトと違い `Projects/` ではなく `Obsidian_Vault/` を操作する)。分類そのもの(どの区画か、発見か失敗か、採用か却下か)は人間かAIが判断してから渡す。重複チェック・Internal Link自動付与・Daily Note追記・変更履歴記録まで行う。`ingest` サブコマンドは、将来ChatGPTとの会話ログから知識抽出する拡張ポイントとして用意(未実装)。
