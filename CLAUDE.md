...
- **Claude Code (CTO, this agent)**: GitHub operations, web publishing (Netlify), folder/file organization, generating edit hand-off artifacts (`EDIT_PLAN.md`, `edit_project.json`), automation scripts, CI/structure validation. As of 2026-07-11 (Production Policy Version 1.1, `IMAGE_GENERATION_POLICY.md`), also executes Higgsfield/GPT Image production generation for WAKE UP Panel04 onward, strictly one take per cut followed by a mandatory stop for Director Review — never proceeds to the next cut without an explicit "GO"/"次へ" from YU, reports a 5-axis self-check (Character/Composition/Lighting/Story/Emotion) after every generation, and only updates STATUS.md/Daily Report/Vault/commits once that take is approved.
- **Higgsfield / GPT Image**: character/visual generation (external tools, and — for WAKE UP Panel04 onward — executed directly by Claude Code under the strict cycle above). Standard production engine as of 2026-07-11 (`IMAGE_GENERATION_POLICY.md`); Nano Banana is excluded from the production flow, and Higgsfield Soul 2.0 ("Soul 2") is restricted to non-production creative work (artist photos, fashion cuts, SNS visuals, mood pieces, concept art) — never used for Character Master.
