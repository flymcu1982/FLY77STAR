---
aliases: [Wake Up, WAKE UP]
type: project
---

# WAKE UP
（SE77NTH.のデビューMV。レトロダイナー×スパイミッション構造）

## 概要

- アーティスト: [[10_Characters/SE77NTH|SE77NTH.]]
- 楽曲: 3:19 / BPM 95.7 / 女性R&Bボーカル+男性ラップ
- 脚本: v1.7（31カット構成、BPM同期）
- コンセプト: ギャルの主人公（[[10_Characters/MIU|MIU]]）がアパレル店「FLY77STAR. CLOSET」閉店後、レトロダイナー「POP DINER」で眠り、ウェイトレスになる夢を見る。Last Chorusで3人が揃いのウェイトレス衣装（メンバーごとに3色）+ローラースケートで群舞
- 参照演出: Limp Bizkit "Take a Look Around"（潜入/電話のブックエンド/広角アティチュード）
- ブランドは実在名を使わず架空代替（POP DINER等）

## 次にやること

- [ ] ダイナー小物アセット(テーブル/メニュー/グラス)のコンセプト確認
- [ ] 3色ウェイトレスユニフォーム(MIU/AYA/NANA 別カラー)の配色案
- [ ] ローラースケート 動きシート生成テスト
- [ ] 31カットのフレーム計算 最終確認(BPM 95.7)(GitHub側 `project.json` にはcutCount:31として登録済みだが、これは頭数の登録のみ。フレーム計算そのものはまだ未確認)
- [ ] CUT01〜15の画像生成 + Palmier素材セット

## 関連

- Knowledge:
- Projects:
- Characters: [[10_Characters/SE77NTH|SE77NTH.]], [[10_Characters/MIU|MIU]], [[10_Characters/AYA|AYA]], [[10_Characters/NANA|NANA]]
- Prompt:
- GitHub(正本): `Projects/WAKE_UP/`
